...
export function seccionMeta(g: Partial<Growth>, huecos: Record<string, string>): string {
  ...
  if (!s) {
    return `${cab}
      <div style="margin-top:20px;">${campanas}</div>
      <div style="margin-top:18px;">${hueco(huecos.segmentacion ?? 'La segmentación detallada no se generó.')}</div>`;
  }
    ...
    <div class="card ${i === 0 ? 'card-pink' : 'card-blue'}">
      <div class="persona-hd">
        ...
        <div>
          ...
          <span class="badge b-gray" style="margin-top:6px;">${escapar(p.etiqueta)}</span>
        </div>
      </div>
      ${p.campos.map((c) =>
  ...
  const capas = `<div class="g3">${s.capas.map((c, i) => `
    <div class="card">
      ...
      ${c.nota ? `<p class="tiny" style="margin-top:10px;">${escapar(c.nota)}</p>` : ''}
    </div>`).join('')}</div>`;

  return `${cab}
    <div style="margin-top:20px;">${campanas}</div>

    <h3 style="margin-top:28px;">Perfil del target</h3>
    ${perfiles}
    <div class="alert alert-green" style="margin-top:16px;">${escapar(s.notaSegmentacion)}</div>

    <h3 style="margin-top:28px;">Configuración por campaña</h3>
    ${config}

    <h3 style="margin-top:28px;">Segmentación detallada · intereses y comportamientos</h3>
    ...
    <div class="g2" style="margin-top:20px;">
      ...
      <div class="card card-pink">
        ...
        <div style="margin-top:10px;">${listaGrowth(s.noSegmentar, 'lst lst-x')}</div>
      </div>
    </div>

    <h3 style="margin-top:28px;">Audiencias personalizadas y similares</h3>
