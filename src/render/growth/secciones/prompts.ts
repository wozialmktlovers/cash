...
export function seccionPrompts(g: Partial<Growth>, huecos: Record<string, string>): string {
  ...
  const cuerpo = p
    ? `
      <div class="alert alert-red" style="margin-top:18px;">
      ...
      <h3 style="margin-top:24px;">Prompt base · anteponer a todos</h3>
      <div class="pre">${escapar(p.base)}</div>
      <h3 style="margin-top:24px;">Uno por pieza</h3>
    ...
    : `<div style="margin-top:18px;">${hueco(huecos.prompts ?? 'Los prompts no se generaron.')}</div>`;
