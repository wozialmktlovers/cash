...
function panelCompartir(o: OpcionesBarra): string {
  ...
  const regenerar = `/clientes/${escapar(o.clienteId)}/investigar`;
  ...
  return `<div class="panel-compartir" id="panel-compartir" role="dialog" aria-label="Compartir investigación" hidden>
    ...
    <button type="button" class="panel-boton panel-primario" id="panel-crear" data-documento="${escapar(o.documentoId)}" ${o.tokenActivo ? 'hidden' : ''}>Crear link público</button>
    <div class="panel-filas">
      <a class="panel-boton" href="/clientes/${escapar(o.clienteId)}">Cliente</a>
      <a class="panel-boton" href="${regenerar}">Regenerar</a>
...
/**
 * Cabecera flotante del documento: cápsula fija con logo, título, switch de
 * tema, progreso de lectura y, solo en la vista interna (cuando se pasan
 * `operador`), el botón Compartir con su panel. Sustituye a las dos barras
 * de antes (`.doc-barra` del documento y `#barra-op` del operador).
 */
...
${operador ? panelCompartir(operador) : ''}`;
}
 ...
 * (crear/copiar/revocar sobre /api/share).
 */
  ...
  if (crear) {
    crear.addEventListener('click', function () {
      ...
      fetch('/api/share', {
        ...
        body: JSON.stringify({ resultId: crear.getAttribute('data-documento'), tipo: 'research' })
