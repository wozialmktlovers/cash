...
function tarjetaCompetidor(c: Competidor, ruta: string, editable: boolean): string {
...
function panelCompetencia(c: Competencia | null, cliente: string, editable: boolean): string {
  ...
  return `${graficaBarras(filas, 'Precio de cada opción')}
    ${c.directos.length ? `<div class="pila"><h4>Competencia directa</h4><div class="rejilla tres">${c.directos.map((x, i) => tarjetaCompetidor(x, `competencia.datos.directos.${i}`, editable)).join('')}</div></div>` : ''}
    ${c.indirectos.length ? `<div class="pila"><h4>Otras opciones que compiten por el mismo presupuesto</h4><div class="rejilla tres">${c.indirectos.map((x, i) => tarjetaCompetidor(x, `competencia.datos.indirectos.${i}`, editable)).join('')}</div></div>` : ''}
...
export function seccionDetalle(inv: Investigacion, cliente: string, num: string, editable = false): string {
  const contenido: Record<(typeof TEMAS)[number]['clave'], string> = {
    competencia: panelCompetencia(datos(inv.competencia), cliente, editable),
...
export function sintesisEditorial(s: Sintesis | null, num: string, editable = false): string {
  if (!s) return '';
  ...
  return `<section class="seccion" id="sintesis" data-seccion>
    ...
    <div class="rejilla dos">${s.hallazgos.map((h, i) => `<article class="tarjeta aparece"><h3${rutaEditable(editable, `sintesis.datos.hallazgos.${i}.titulo`)}>${escapar(h.titulo)}</h3><p${rutaEditable(editable, `sintesis.datos.hallazgos.${i}.texto`)}>${escapar(h.texto)}</p></article>`).join('')}</div>
    ...
    <div class="rejilla tres">${s.focos.map((f, i) => `<article class="tarjeta aparece"><p class="eyebrow">${escapar(tipos[f.tipo] ?? f.tipo)}</p><h3${rutaEditable(editable, `sintesis.datos.focos.${i}.nombre`)}>${escapar(f.nombre)}</h3><p${rutaEditable(editable, `sintesis.datos.focos.${i}.razon`)}>${escapar(f.razon)}</p></article>`).join('')}</div>
  </section>`;
}
