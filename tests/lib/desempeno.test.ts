...
// Se importa para comprobar que el % del tablero y el del portal siguen
// dando lo mismo para el mismo cliente (M3, controlador punto 6).
import { avanceCliente } from '@/flujo/reglas';
...
describe('carga', () => {
  ...
  it('desarrollo_mensual contratado cuenta en el avance, igual que avanceCliente (M3, controlador punto 6)', () => {
    // Lo que se prueba aquí no es el número sino que carga() y avanceCliente
    // sigan de acuerdo: comparten `etapasParaAvance` justo para que el
    // tablero de desempeño y el portal no muestren % distintos del mismo
    // cliente. A3 metió desarrollo_mensual al avance en un solo lugar y este
    // módulo lo heredó sin tocarlo.
    const etapas = [
    ...
    // (100+0)/2 = 50. Con la exclusión de antes: 100/1 = 100.
    expect(carga(etapas, [], p).avancePromedio).toBe(50);
    expect(carga(etapas, [], p).avancePromedio).toBe(avanceCliente(etapas));
  });
});
